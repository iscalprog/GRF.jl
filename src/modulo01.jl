...
function exce��es(retornos, alfa)
...
function gfe(mu, Sigma, z_alfa, T, V_0, ncarteiras = 10)
    ...
    fig = scatter!(aVaR, mu, label = "A��es")
    VaR_CVM = -z_alfa * sqrt(T) * sqrt(w_CVM' * Sigma * w_CVM) * V_0
    fig = scatter!([VaR_CVM], [mu_CVM], label = "Carteira VaR M�nimo")
    return fig
end

function aloca��es(mu, Sigma, lista, ncarteiras = 10)
    modelo = Model(Ipopt.Optimizer)
    set_silent(modelo)       
    n = length(mu)                           
    @variable(modelo, w[1:n])                           
    ...
    fig = groupedbar(w_k', bar_position = :stack, bar_width=1.0, xlabel = "Carteiras", xticks=(1:ncarteiras, ticklabel), ylabel = "Aloca��es", label = permutedims(String.(lista)), legend = :bottomright)
    return fig
end
